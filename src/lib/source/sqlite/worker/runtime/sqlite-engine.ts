...
export const getSqlite3 = (): Promise<Sqlite3Static> => {
	if (!sqlite3Promise) {
		// Suppress OPFS warning that occurs when SQLite tries to initialize OPFS
		// VFS on the main thread.
...
export const openDatabase = (
	...
	// SQLite header bytes 18-19 indicate journal mode: 1 rollback, 2 WAL.
	// WAL databases fail after in-memory deserialize without -wal/-shm sidecars,
	// so we patch to rollback mode for immediate queryability.
	if (bytes[18] === 2) {
		bytes[18] = 1
		bytes[19] = 1
	}
...
export const forEachRow = async (
	...
	if (params.length > 0) statement.bind(params as BindingSpec)
	let processed = 0

	try {
		while (statement.step()) {
			onRow(statement.get({}) as SqlRow)
			processed += 1
